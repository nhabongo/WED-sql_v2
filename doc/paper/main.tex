...
Cada instância de um WED-flow pode ser vista como uma Transação Longa (LLT) e, desse modo, de acordo com ~\cite{SGD87},
as WED-transitions podem ser vistas como passos Saga. Sendo assim, é função do WED-server garantir a integridade dos estados
...
%locks are needed for: 1-keep track of wed-trans; 2-concurrency control between wed-workers (pg_locks,
%DETALHES DE FUNCIONAMENTO (locks(pg_row_locks),keep connection alive(pessimistic locking),excessoes, tras simultaneas,continuous query,protocolo de comunicaocao, balanço: coneccoes x demanda,payload pode nao ser o estado atual mvcc)
